...
4. Archive existing `PRD.md` → `PRD/PRD-<title>.md` and `progress.txt` → `progress/progress-<date>.txt` if they exist
5. Save new `PRD.md`
...
### Before saving, archive any existing files:

If `PRD.md` exists:
1. Create a `PRD/` folder if it doesn't exist
2. Read the first `# ` heading from `PRD.md` to get the title (e.g. `# PRD: Task Priority System` → `task-priority-system`)
3. Move `PRD.md` → `PRD/PRD-<slugified-title>.md`

If `progress.txt` exists:
1. Create a `progress/` folder if it doesn't exist
2. Move `progress.txt` → `progress/progress-<YYYY-MM-DD>.txt` (use today's date)
...
- [ ] Archived existing PRD.md and progress.txt if they existed
